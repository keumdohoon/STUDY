...
x_train = pd.read_csv('./data/csv/loan_traintest_data.csv')
x_pred = pd.read_csv('./data/csv/loan_prediction_data.csv')
...
for col in x_train.columns :
    listItem.append([col,x_train[col].dtype,
                     x_train[col].isna().sum(),
                     round((x_train[col].isna().sum()/len(x_train[col])) * 100,2),
                    x_train[col].nunique(),
                     list(x_train[col].sample(5).drop_duplicates().values)]);
...
#데이터에 있는 x를 education을 기준으로 그룹바이 시켜주고 experience의 평균만을 가져와서 비교해준다. 
pd.DataFrame(x_train.groupby("Education").mean()["Experience"])




#데이터에 있는 x를 age을 기준으로 그룹바이 시켜주고 experience의 평균의 마지막 8개를 가져와서 비교해준다. 
pd.DataFrame(x_train.groupby("Age").mean()["Experience"]).tail(8)
#age와 exp를 가지고 플롯을 그려준다
pltdf = pd.DataFrame(x_train.groupby("Age").mean()["Experience"]).reset_index()
sns.lmplot(x='Age',y='Experience',data=pltdf)
plt.ylabel("Experience (Average)")
plt.title("Average of Experience by Age")
plt.show()

pd.DataFrame(x_train[x_train["Experience"]<0][["Age","Experience"]].sort_values("Age")).head()

pd.DataFrame(x_train[x_train["Experience"]<0][["Age","Experience"]].sort_values("Age"))["Experience"].unique()
#feature1에서 결측치를 핸들링하는 방법
x_train["Experience"] = x_train["Experience"].apply(abs)
...
x_train[["Age","Experience","Income","CCAvg","Mortgage"]] = x_train[["Age","Experience","Income","CCAvg","Mortgage"]].astype(float)
print(x_train.info())
   #   Column              Non-Null Count  Dtype
 #   ---  ------              --------------  -----
...
x_train_corr = x_train.join(x_test).corr()
...
with sns.axes_style("white"):
    ...
    sns.heatmap(x_train_corr, annot=True, square=True, mask=mask)
# plt.show()

sns.distplot(y["Mortgage"])
plt.title("Mortgage Distribution with KDE")
# plt.show()

# Irregular value handling feature 2 (extreme positive skewed data)
SingleLog_y = np.log1p(y["Mortgage"])              
...
x_train["Mortgage"] = DoubleLog_y

source_counts =pd.DataFrame(x["Personal Loan"].value_counts()).reset_index_train()
...
sns.distplot(x_train[x_train["Personal Loan"] == 0]['Income'], color = 'r',label='Personal Loan=0',kde=False)
sns.distplot(x_train[x_train["Personal Loan"] == 1]['Income'], color = 'b',label='Personal Loan=1',kde=False)
plt.legend()
plt.title("Income Distribution")
plt.show()

plt.figure(figsize=(10,6))
sns.distplot(x_train[x_train["Personal Loan"] == 0]['CCAvg'], color = 'r',label='Personal Loan=0',kde=False)
sns.distplot(x_train[x_train["Personal Loan"] == 1]['CCAvg'], color = 'b',label='Personal Loan=1',kde=False)
plt.legend()
...
plt.figure(figsize=(10,6))
sns.distplot(x_train[x_train["Personal Loan"] == 0]['Experience'], color = 'r',label='Personal Loan=0',kde=False)
sns.distplot(x_train[x_train["Personal Loan"] == 1]['Experience'], color = 'b',label='Personal Loan=1',kde=False)
plt.legend()
plt.title("Experience Distribution")
plt.show()


sns.countplot(x_train='Securities Account',data=x_train,hue='Personal Loan')
plt.title("Securities Account Countplot")
plt.show()

sns.countplot(x_train='Family',data=x_train,hue='Personal Loan')
plt.title("Family Countplot")
plt.show()


sns.boxplot(x_train='Education',data=x_train,hue='Personal Loan',y='Income')
plt.legend(loc='lower right')
plt.title("Education and Income Boxplot")
plt.show()

sns.boxplot(x_train='Family',data=x_train,hue='Personal Loan',y='Income')
...
#features removing
#feature = feature.drop(["ZIP Code","Age","Online","CD Account"],axis=1)
...
predict = xgb.predict(x_pred)
predictProb = xgb.predict_proba(x_pred)
print('predictprob',predictProb)
