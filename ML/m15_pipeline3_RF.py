...
parameter = [{"randomforestclassifier__n_jobs":[1]}]#,'randomforestclassifier__kernel':['linear']}]#, 
             #{"randomforestclassifier__C":[1, 10, 100, 1000], 'svm__kernel':['rbf'], 
             #                            'svm__gamma':[0.001, 0.0001]},   
             #{"randomforestclassifier__C":[1, 10, 100, 1000], 'svm__kernel':['sigmoid'],
             #                            'svm__gamma':[0.001, 0.0001]}
...
# make pipeline은 (전처리, 모델) 바로 명시를 해준다. 
...
#통상 그리드 서치를 할때에는 그리드 서치 명만 적어주면 된다. 하지만 파이프라인으로 엮게 된다면 앞에다가 모델명이랑 __언더라인 두개를 추가해줘야한다.
