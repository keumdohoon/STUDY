...
cp = ModelCheckpoint(filepath='./model/sample/cifar100/{epoch:02d}-{val_loss:.4f}.hdf5', monitor='val_loss', save_best_only=True, mode='auto')

tb = TensorBoard(log_dir='graph', histogram_freq=0, write_graph=True, write_images=True)

### 3. 훈련
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc'])
hist = model.fit(x_train, y_train, epochs=30, batch_size=42, verbose=1, validation_split=0.2, callbacks=[es, cp, tb])
model.save('./model/sample/cifar100/cifar100_model_save.h5')
model.save_weights('./model/sample/cifar100/cifar100_model_save_weights.h5')
