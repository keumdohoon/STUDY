...
hist = model.fit(x, y, epochs=30, validation_split=0.2, batch_size=1, verbose=1,
           callbacks=[early_stopping])
           #변수를 이미 만들어 놓으면 나중에 리스트에서 푸펀으로  earlystopping이 이미 나오게 되어있다. 
#hist 는 임의의 단어이기 때문에 그 무엇과도 바꿀수가 있다. dictionary측에서 나온 hist라는 말이다. dictionary의 key를 내어주는 것이다. 
#딕셔너리의 key는 value와 한 쌍인디...
print(hist)#보여주지 않기 때문에 과정값을 보기에 가장 쉬운방법 그래프로 보는것이 결국에는 방법이다. 
