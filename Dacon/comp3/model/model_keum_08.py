...
X_data = np.loadtxt('./dacon/comp3/data/train_features.csv',skiprows=1,delimiter=',')
X_data = X_data[:,1:]
print(X_data.shape)
     
    
Y_data = np.loadtxt('./dacon/comp3/data/train_target.csv',skiprows=1,delimiter=',')
Y_data = Y_data[:,1:]
print(Y_data.shape)
...
X_data_test = np.loadtxt('./dacon/comp3/data/test_features.csv',skiprows=1,delimiter=',')
...
def set_model(train_target):  # 0:x,y, 1:m, 2:v
    ...
    fs1 = (4,1)
    fs2 = (3,1)
    model.add(Conv2D(nf,fs1, padding=padding, activation=activation,input_shape=(375,5,1)))
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2, 1)))

    model.add(Conv2D(nf*2,fs2, padding=padding, activation=activation))
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2, 1)))

    model.add(Conv2D(nf*4,fs1, padding=padding, activation=activation))
    model.add(BatchNormalization())

    model.add(Conv2D(nf*8,fs2, padding=padding, activation=activation))
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(3, 1)))

    model.add(Conv2D(nf*16,fs1, padding=padding, activation=activation))
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2, 1)))

    model.add(Conv2D(nf*32,fs2, padding=padding, activation=activation))
    model.add(BatchNormalization())
...
submit = pd.read_csv('./dacon/comp3/data/sample_submission.csv')
...
submit.to_csv('./dacon/comp3/sub/comp_submit111.csv', index = False)
