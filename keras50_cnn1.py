...
#model.add(Conv2D(5, (2,2)), stride=2, padding='same')#이렇게 되면 패딩값보다 스트라이드 값을 먼저 쳐준다. 
...
# stride(띄어주는, 몇개씩점프를 하는지) 것 은 항상 디폴트값 1로 되어있다. 
# model.add(Conv2D(10, kernel_size=2, input_shape = (5,5,1)))
#maxpooling이란 전체 그림에서 가장 중요한 곳들만을 빼와서 하나를 만들어 주는 것이다 만들때 풀사이즈는 2이다. 
